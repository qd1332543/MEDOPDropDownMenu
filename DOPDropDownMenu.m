...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextSetRGBStrokeColor(context, 219.0/255, 219.0/255, 219.0/255, 1);//线条颜色
...
@interface DOPDropDownMenu ()<MEDOPDropDownMenuCellDelegate>{
struct {
...
@property (nonatomic, assign) BOOL isTitleRect;     // 适用于标题有1个，且指定标题frame的情况
...
@property (nonatomic, assign) NSInteger selectSortRow;
@property (nonatomic, strong) UIImage *tmpSortImage;
@property (nonatomic, assign) BOOL isEqualSelected;
...
#define kIndicatorColor [UIColor colorWithRed:204/255.0 green:204/255.0 blue:204/255.0 alpha:1]
#define kTheMainColor [UIColor colorWithRed:255/255.0 green:69/255.0 blue:93/255.0 alpha:1]
...
- (UIColor *)indicatorColor {
    if (!_indicatorColor) {
        _indicatorColor = kIndicatorColor;
    }
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        _textSelectedColor = kTheMainColor;
        _indicatorColor = kIndicatorColor;
        ...
        _leftTableView.backgroundColor = ME_LIGHTLIGHTGRAY;
        [self bringSubviewToFront:_leftTableView];
        
        _separatorView = [[UIView alloc] initWithFrame:CGRectMake(screenSize.width / 2, 0, 0.5, 0)];
        _separatorView.backgroundColor = ME_GRAY_153;
        ...
        _rightTableView.backgroundColor = MEAlphaColor(238, 238, 238, 0.9);
        [self bringSubviewToFront:_rightTableView];
...
- (instancetype)initWithMenuFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        //lefttableView init
        _leftTableView = [[UITableView alloc] initWithFrame:CGRectMake(_origin.x, _origin.y + frame.size.height, screenSize.width/2, 0) style:UITableViewStylePlain];
        _leftTableView.rowHeight = kTableViewCellHeight;
        _leftTableView.dataSource = self;
        _leftTableView.delegate = self;
        _leftTableView.separatorColor = kSeparatorColor;
        ...
        //righttableView init
        _rightTableView = [[UITableView alloc] initWithFrame:CGRectMake(_origin.x + screenSize.width/2, _origin.y + frame.size.height, screenSize.width/2, 0) style:UITableViewStylePlain];
        _rightTableView.rowHeight = kTableViewCellHeight;
        _rightTableView.dataSource = self;
        _rightTableView.delegate = self;
        _rightTableView.separatorColor = kSeparatorColor;
        ...
        _buttomImageView = [[UIImageView alloc]initWithFrame:CGRectMake(_origin.x, frame.origin.y + frame.size.height, self.frame.size.width, kButtomImageViewHeight)];
        _buttomImageView.image = [UIImage imageNamed:@"icon_chose_bottom"];
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
    ...
    // 调用代理方法通知VC界面
    [self.scrollDelegate gestureClickResponder];
...
/**
 *  判断菜单的跟视图,默认为keyWindow
 */
-  (UIView *)containerView{
    
    if (_isClickGesture) {
        NSLog(@"%@",_menuContainerView.superview);
...
- (void)animateIndicator:(CAShapeLayer *)indicator Forward:(BOOL)forward complete:(void(^)())complete {
    ...
    if (forward) {
        // 展开
        indicator.fillColor = kTheMainColor.CGColor;
    } else {
        // 收缩
        indicator.fillColor = kIndicatorColor.CGColor;
    }
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
        if (haveItems) {
            ...
            _separatorView.frame = CGRectMake(self.origin.x + self.frame.size.width/2, self.frame.origin.y + self.frame.size.height, 0.5, 0);
            _rightTableView.frame = CGRectMake(self.origin.x + self.frame.size.width/2+0.5, self.frame.origin.y + self.frame.size.height, self.frame.size.width/2-0.5, 0);
            [self.superview addSubview:_leftTableView];
            [self.superview addSubview:_separatorView];
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
                 ...
                 _separatorView.frame = CGRectMake(self.origin.x + self.frame.size.width/2, self.frame.origin.y + self.frame.size.height, 0.5, tableViewHeight);
                 _rightTableView.frame = CGRectMake(self.origin.x + self.frame.size.width/2+0.5, self.frame.origin.y + self.frame.size.height, self.frame.size.width/2-0.5, tableViewHeight);
            } else {
...
{
    MEDOPDropDownMenuCell *cell = [MEDOPDropDownMenuCell cellWithTableView:tableView];
    DOPBackgroundCellView *bg = [[DOPBackgroundCellView alloc]init];
    bg.backgroundColor = [UIColor whiteColor];
    cell.selectedBackgroundView = bg;
    cell.dropDownMenuCellTitleLabel.textColor = kTextColor;
    cell.dropDownMenuCellTitleLabel.highlightedTextColor = kTheMainColor;
    if (_tmpSortImage) {
        if (_selectSortRow == indexPath.row) {
            cell.dropDownMenuCellTitleImageView.image = _tmpSortImage;
        } else {
            cell.dropDownMenuCellTitleImageView.image = [UIImage imageNamed:@"ascending"];
        }
    } else {
        cell.dropDownMenuCellTitleImageView.image = [UIImage imageNamed:@"ascending"];
    }
    ...
    if (tableView == _leftTableView) {
        if (_dataSourceFlags.titleForRowAtIndexPath) {
            cell.dropDownMenuCellTitleLabel.text = [_dataSource menu:self titleForRowAtIndexPath:[DOPIndexPath indexPathWithCol:_currentSelectedMenudIndex row:indexPath.row]];
            cell.dropDownMenuCellTitleLabel.textAlignment = NSTextAlignmentCenter;

            if (!_isSort) {
                [cell.dropDownMenuCellTitleImageView setHidden:YES];
            } else {
                if (0 == indexPath.row) {
                    [cell.dropDownMenuCellTitleImageView setHidden:YES];
                }
        ...
        if ([cell.dropDownMenuCellTitleLabel.text isEqualToString:[(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string]]) {
        ...
        if (_dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:indexPath.row column:_currentSelectedMenudIndex]> 0){
            ...
            cell.dropDownMenuCellTitleLabel.textAlignment = NSTextAlignmentLeft;
            [cell.dropDownMenuCellTitleImageView setHidden:YES];
            cell.labelToContentViewLeadingConstraint.constant = 5;
        } else {
        ...
        if (_dataSourceFlags.titleForItemsInRowAtIndexPath) {
            cell.dropDownMenuCellTitleLabel.text = [_dataSource menu:self titleForItemsInRowAtIndexPath:[DOPIndexPath indexPathWithCol:_currentSelectedMenudIndex row:_currentSelectedMenudRow item:indexPath.row]];
        } else {
            NSAssert(0 == 1, @"dataSource method needs to be implemented");
        }
    ...
    cell.dropDownMenuCellTitleLabel.font = [UIFont systemFontOfSize:_fontSize];
    return cell;
}
    ...
    if (_leftTableView == tableView) {
        ...
        if (isClickHaveItemValid && _delegate && [_delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
            ...
            if (_currentSelectedMenudIndex == 1) {
                if (indexPath.row != 0) {
                    [self changeCellSortImageWithRow:indexPath.row];
                }
            }
        } else {
...
- (void)changeCellSortImageWithRow:(NSInteger)row {
    if (row) {
        if (row == _selectSortRow) {
            if (_isEqualSelected == YES) {
                _tmpSortImage = [UIImage imageNamed:@"descending"];
                _isEqualSelected = NO;
                _selectSortRow = row;
            } else {
                _tmpSortImage = [UIImage imageNamed:@"ascending_main"];
                _isEqualSelected = YES;
                _selectSortRow = row;
            }
        } else {
            _tmpSortImage = [UIImage imageNamed:@"ascending_main"];
